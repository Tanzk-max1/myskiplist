...
完成跳表的创建之后，接下来完成跳表的搜索方法。

搜索开始于跳表的顶层，这一点在我们的 SkipList 类中通过变量 _skip_list_level 得到体现，该变量记录了跳表当前的最高层级。

每个节点都维护一个 forward 数组，该数组记录了该节点在每一层的下一个节点的指针。
...
```
是用于实现插入节点的关键数据结构，它是一个节点指针数组，用于记录在上文中提到的，待插入节点的前驱节点（即在该层中小于新节点且最接近新节点的节点）。这个数组解决了之前提到的关键问题：在插入新节点时如何更新每层的指针关系。
...
用update数组记录每层待删除的节点的前驱节点，以便更新指针关系。
...
跳表的结构本质上是通过对原始链表的部分节点进行筛二构建的多级索引链表，可以视为多个层级的单链表的结合。

跳表的每一层都有一个头节点，通过这些头结点可以访问到该层的所有节点，从而实现对每一层的访问。

### 2.代码逻辑

利用跳表的头结点数组_header,其中_header[i]代表第i层的头结点。我们来进行对每一层头结点的遍历
